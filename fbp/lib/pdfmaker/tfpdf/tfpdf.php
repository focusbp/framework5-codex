...
var $page_max_height;     // [$this->page]ページ内の最大高さ（morepagetableで使用）

var $hf; //bufferにするためのファイルポインタ

var $imgdir; //画像ディレクトリ（配列）
...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	$cell_margin = 0.2; //cm
	$this->cMargin = $this->CSMargin_x =  $this->CSMargin_y = 28.35*$cell_margin/$this->k;
	// Line width (0.2 mm)
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			...
			$s.='$ut='.$ut.";\n";
			//$s.='$ttffile=\''.$ttffile."';\n";
			$s.='$ttffile= __DIR__ . ' . '\'/'. basename($ttffile) ."';\n";
				...
				
{
	...
	if($fill || $border==1)
	{
		if($fill)
			...
			$op = 'S';
		$s = sprintf('%.2F %.2F %.2F %.2F re %s ',$this->x*$k,($this->h-$this->y)*$k,$w*$k,-($h+$this->CSMargin_x*2)*$k,$op);
	}
	if(is_string($border))
	{
		$x = $this->x;
		...
		if(strpos($border,'L')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',$x*$k,($this->h-$y)*$k,$x*$k,($this->h-($y+$h+$this->CSMargin_x*2))*$k);
		if(strpos($border,'T')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',$x*$k,($this->h-$y)*$k,($x+$w)*$k,($this->h-$y)*$k);
		if(strpos($border,'R')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',($x+$w)*$k,($this->h-$y)*$k,($x+$w)*$k,($this->h-($y+$h+$this->CSMargin_x*2))*$k);
		if(strpos($border,'B')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',$x*$k,($this->h-($y+$h+$this->CSMargin_x*2))*$k,($x+$w)*$k,($this->h-($y+$h+$this->CSMargin_y*2))*$k);
	}
	
	//$wを変更
	$w = $w - $this->CSMargin_x*2;
	
	if($txt!=='')
	{
		$dy = $this->CSMargin_y;
		if($align=='R')
			$dx = $w-$this->GetStringWidth($txt)+$this->CSMargin_x;
		elseif($align=='C')
			$dx = ($w-$this->GetStringWidth($txt))/2 +$this->CSMargin_x;
		else
			$dx = $this->CSMargin_x;
		if($this->ColorFlag)
			$s .= 'q '.$this->TextColor.' ';
...
//			$tc = 0;
//			$s .= sprintf('BT %.2F %.2F Td %s Tc %s Tj ET',($this->x+$dx)*$k,($this->h - ($this->y+$dy+ .5*$h + .3*$this->FontSize))*$k,$tc*$k,$txt2);
			
			//自動文字間調整
			$str_width = $this->GetStringWidth($txt);
			$str_count = mb_strlen($txt);
			$str_avg = $str_width/$str_count;
			$sabun = ($w - $str_avg) - $str_width;
			if($str_width/$w > 0.90){
				$tc = $sabun/$str_count;
			}else{
				$tc = 0;
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $span_x=0,$span_y=0,$flg_dulble_line=true)
{
	...
	// spanを設定
	$this->CSMargin_x = $span_x + $this->cMargin;
	$this->CSMargin_y = $span_y + $this->cMargin;
	
	// Output text with automatic or explicit line breaks
	$cw = &$this->CurrentFont['cw'];
	if($w==0)
		$w = $this->w - $this->rMargin - $this->x;
	$wmax = ($w-2*$this->CSMargin_x);
	$s = str_replace("\r",'',$txt);
	
	// 画像の取得
	$imagefilename = false;
	if(is_array($s) || strpos($s, "\[image:") !== false){
		if(is_array($s)){
			$imagefilename = $s["path"];
		}else{
			$image_start_pos = strpos($s, "\[image:");
			$image_end_pos = strpos($s,"]",$image_start_pos);
			$imgtag=substr($s,$image_start_pos,$image_end_pos - $image_start_pos);
			$imagefilename = substr($imgtag,8,strlen($imgtag) - 8);                 //画像ファイルを取得
		}
		
		// 画像処理
		foreach($this->imgdir as $idir){
			$img = $idir . $imagefilename;
			if(file_exists($img)){
				$x = $this->x;
				// Just check height
				$ret = $this->Image($img, $this->x+$this->CSMargin_x, $this->y+$this->CSMargin_y, $w-2*$this->CSMargin_x,0,"","",true);
				$h = $ret["h"];
				
				if($this->y+$h+$this->cMargin*2 > $this->PageBreakTrigger &&$this->AcceptPageBreak())
				{
					// Automatic page break
					$x2 = $this->x;
					$this->AddPage($this->CurOrientation,$this->CurPageSize);
				...
				// 実際に描画
				$this->Image($img, $this->x+$this->CSMargin_x, $this->y+$this->CSMargin_y, $w-2*$this->CSMargin_x,0,"","");
				$this->x = $x;
			}
		}
		$s="";
	...
	while($i<$nb)
	{
		...
		// 禁則処理 2018.1.25 中間
		$kinsoku = ",)）]｝、〕〉》」』】〙〗〟’”｠»ゝゞーァィゥェォッャュョヮヵヶぁぃぅぇぉっゃゅょゎゕゖㇰㇱㇲㇳㇴㇵㇶㇷㇸㇹㇷ゚ㇺㇻㇼㇽㇾㇿ々〻‐゠–〜～?!‼⁇⁈⁉・:;/。.";
		$last_str = mb_substr($c,-1);
		if(strpos($kinsoku,$last_str) !== false){
			$kinsoku_flg = false;
		}else{
			$kinsoku_flg = true;
		}

		if($kinsoku_flg && $l>=$wmax)
		{
	...
	// 元に戻す
	$this->CSMargin_x = $this->cMargin;
	$this->CSMargin_y = $this->cMargin;
}
...
function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='',$onlycheck=false)
{
...
function SetY($y,$flg_xreset=true)
{
...
function _getpagesize($size)
{
	...
	if(is_string($size))
	{
		...
		if(isset($this->StdPageSizes[$size])){
			...
			if(strpos($size,"mm") >0 && strpos($size,"x") >0){
				// 210x297mm 形式
				$size = str_replace("mm","",$size); //mmを取る
				$ex = explode("x",$size);
				$a = array($ex[0],$ex[1]);
				return $a;
			}else{
				$ex = explode(",",$size);
				//$a = array($ex[0]*2.2895,$ex[1]*2.2895);
				$a = array($ex[0],$ex[1]);
				return $a;
			}
		}
...
function _dounderline($x, $y, $txt)
{
	...
	return sprintf('%.2F %.2F %.2F %.2F re f',$x*$this->k,($this->h-($y+$this->cMargin-$up/1000*$this->FontSize))*$this->k,$w*$this->k,-$ut/1000*$this->FontSizePt);
}
...
function _parsejpg($file)
{
	...
	//あとで読み込む
	return array('w'=>$a[0], 'h'=>$a[1], 'cs'=>$colspace, 'bpc'=>$bpc, 'f'=>'DCTDecode', 'data'=>"JPG",'file'=>$file);
}
...
function _parsepngstream($f, $file)
{
	...
	if($bpc>8){
		...
		$temp_file = tempnam($tmpdir, 'Tux') . ".png";
		exec("convert $file -define png:bit-depth=8 $temp_file",$output,$error);
		$f2 = fopen($temp_file, "r");
		$res = $this->_parsepngstream($f2, $temp_file);
		fclose($f2);
		return $res;
...
function echo_buffer(){
	rewind($this->hf);
	while ($this->hf && !feof($this->hf)) {
		echo fread($this->hf,1024);
	}
}

function write_buffer($f){
	rewind($this->hf);
	while ($this->hf && !feof($this->hf)) {
		$b = fread($this->hf,8192);
		fwrite($f,$b);
	}
}
...
function len_buffer(){
	fseek($this->hf,0,SEEK_END);
	return ftell($this->hf) + 1;
}

function fwrite_buffer($s){
	fseek($this->hf,0,SEEK_END);
	fwrite($this->hf, $s);
}
