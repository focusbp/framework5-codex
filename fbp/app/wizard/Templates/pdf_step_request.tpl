<form id="wizard_pdf_request_form">
	<p style="font-size:13px;color:#374151;margin:0 0 8px 0;">{t key="wizard.pdf.request.description"}</p>
	...
	<p style="font-weight:bold;margin:0 0 4px 0;">{t key="wizard.pdf.request.fields"}</p>
	<div style="max-height:220px;overflow:auto;border:1px solid #d5dbe5;margin-top:0px;margin-bottom:8px;">
		<table style="width:100%;border-collapse:collapse;font-size:12px;">
			...
			<tbody>
				{if $pdf_selected_fields|@count > 0}
					{foreach $pdf_selected_fields as $one}
						<tr>
				...
				{else}
					<tr>
						<td colspan="3" style="border:1px solid #d5dbe5;padding:6px;color:#6b7280;">{t key="wizard.pdf.request.fields_empty"}</td>
					</tr>
				{/if}
			</tbody>
		</table>
	...
	<p style="font-weight:bold;margin:0 0 4px 0;">{t key="wizard.pdf.request.request_text"}</p>
	...
	<div style="margin-top:12px;overflow:auto;">
		<button type="button" class="ajax-link" invoke-function="back_to_pdf_fields" style="float:left;">{t key="common.back"}</button>
		<button type="button" class="ajax-link" invoke-function="submit_pdf_request_next" data-form="wizard_pdf_request_form" style="float:right;">{t key="common.next"}</button>
	</div>
</form>
